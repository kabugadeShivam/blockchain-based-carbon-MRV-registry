...
describe("BlueCarbonRegistry", function () {
  ...
  it("registers a project and anchors evidence hash", async function () {
    ...
    expect(project.name).to.equal("Navi Mumbai Mangrove Pilot");
    expect(project.evidenceHash).to.equal("sha256:project-demo");
  });

  it("allows a verifier to verify MRV and mint project credits", async function () {
    ...
    await registry.connect(owner).submitMRV(1, "sha256:mrv", 100);
    await registry.connect(verifier).verifyMRV(1, 80);

    expect(await registry.balanceOf(owner.address, 1)).to.equal(80);
    const record = await registry.mrvRecords(1);
    expect(record.verified).to.equal(true);
    expect(record.verifiedCredits).to.equal(80);
  });

  it("prevents non-verifiers from approving MRV", async function () {
    const { registry, owner, outsider } = await deploy();
    await registry.connect(owner).registerProject("Test", "mangrove", "0,0", "hash");
    await registry.connect(owner).submitMRV(1, "mrv-hash", 10);

    await expect(registry.connect(outsider).verifyMRV(1, 10)).to.be.reverted;
  });
  ...
  it("allows a credit holder to retire credits", async function () {
    ...
    await registry.connect(owner).retireCredits(1, 4, "Corporate offset retirement");

    expect(await registry.balanceOf(owner.address, 1)).to.equal(6);
  });
});
